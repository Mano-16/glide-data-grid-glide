...
import { direction, getScrollBarWidth, useDebouncedMemo, useEventListener, convertToStringArray, getGroupLevelIndexFromRow } from "../../common/utils.js";
...
import type { DragAndDropState, DrawGridArg } from "./render/draw-grid-arg.js";
...
import { walkColumns, walkRowsInCol } from "./render/data-grid-render.walk.js";
...
const DataGrid: React.ForwardRefRenderFunction<DataGridRef, DataGridProps> = (p, forwardedRef) => {
    ...
    // row: -1 === columnHeader, -2 === groupHeader, -3 === parent group header
    ...
    const getMouseArgsForPosition = React.useCallback(
        (canvas: HTMLCanvasElement, posX: number, posY: number, ev?: MouseEvent | TouchEvent): GridMouseEventArgs => {
            ...
            if (col === -1 || y < 0 || x < 0 || row === undefined || x > width || y > height) {
                ...
                const groupLevelIndex = row <= -2 ? getGroupLevelIndexFromRow(row, groupHeaderLevels) : -1; // count -2 (as leaf group), -3 (parent group), ..
                ...
                if (posX - bounds.x <= edgeDetectionBuffer && previousCol >= 0) {
                    ...
                    result = {
                        ...
                        bounds: bounds,
                        group: convertToStringArray(mappedColumns[previousCol].group)[groupLevelIndex] ?? "",
                        isEdge,
                        shiftKey,
                        ctrlKey,
                        metaKey,
                    ...
                    result = {
                        kind: enableGroups && row <= -2 ? groupHeaderKind : headerKind,
                        group: convertToStringArray(mappedColumns[col].group)[groupLevelIndex] ?? "",
    ...
    const groupHeaderHovered = hCol !== undefined && (hRow ?? 0) <= -2;
