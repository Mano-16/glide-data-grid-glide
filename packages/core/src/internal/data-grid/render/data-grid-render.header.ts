...
import { direction, getGroupLevelIndexFromRow } from "../../../common/utils.js";
...
export function drawGridHeaders(
    ...
    walkColumns(effectiveCols, 0, translateX, 0, totalHeaderHeight, (c, x, _y, clipX) => {
        ...
        const groupTheme = undefined; //getGroupDetails(c.group ?? "").overrideTheme;
        const theme =
...
export function drawGroup(
    ...
    if (group.icon !== undefined) {
        spriteManager.drawSprite(group.icon, "normal", ctx, drawX + xPad, y + (groupHeight - 20) / 2, 20, groupTheme);
        drawX += 26;
    }
